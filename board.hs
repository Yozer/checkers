...
hashFunction square figure = hashTable V.! index
  where
    ...
    index = square*4 + val + 1

hashFunction' :: Word64 -> Board -> Word64
hashFunction' index board = hashFunction (rfield index) (getPiece board index)

flipPlayerHash :: Word64
flipPlayerHash = (hashTable V.! 0) 

hashBoard :: Board -> Player -> Word64
hashBoard board player = result'
  where
      result = foldl xor  (0 :: Word64) . map (\(a,b) -> hashFunction (rfield a) b) . filter (\(_,a) -> a /= Empty) $ zip (getBoardFields [1..32]) (map (getPiece board) $ getBoardFields [1..32])
...
doMove (GameState board player hash) (NormalMove move) = GameState board' (getNextPlayer player) hash'
  where
    from = head move
    to = last move
...
doMove (GameState board player hash) (JumpMove jump) = GameState board' (getNextPlayer player) hash''
  where
    (board', hash') = doJump' (board, hash) player jump
    a = head jump
    b = last jump
    hash'' = (hash' `xor` flipPlayerHash) `xor` (hashFunction' a board) `xor ` (hashFunction' b board')
doMove g _ = g

doJump' :: (Board, Word64) -> Player -> Path -> (Board, Word64)
doJump' (board, hash) player (a:path)
  | path == [] = (board', hash)
  | otherwise = doJump' (result, hash') player path
  where
    b = head path
    ...
    hash' = hash `xor` (hashFunction' killed board)

    promotedKings = promotePiece a player (k board)
    board' = board {k = promotedKings}
